...
MQTT_PUBLISH_COUNTER = Counter('edge_mqtt_publish_total', 'MQTT messages published')
...
class EdgeDevice:
    def __init__(self):
        ...
        logger.info(f"Initializing Edge Device: {self.branch_name} ({self.branch_id})")
    ...
    def process_customer(self):
        ...
        try:
            ...
            # Step 2: Detect face and extract attributes
            ...
            self.mqtt_client.publish_recommendation(message)
            ...
            if purchased_items:
                ...
                self.mqtt_client.publish_transaction(transaction_data)
                self.transaction_count += 1
                
                logger.info(f"Transaction completed: {len(purchased_items)} items, "
                           f"Total: {transaction_data['total_amount']:.2f} VND")
            
            # Measure latency
            latency = time.time() - start_time
            INFERENCE_LATENCY.observe(latency)
            
            logger.info(f"Customer processed in {latency*1000:.2f}ms")
                ...
                purchased = [
                    {
                        ...
                        'category': random.choice(['Beverage', 'Snack', 'Personal Care', 'Dairy'])
                    }
    ...
    def run(self):
        """Main event loop"""
        self.running = True
        logger.info(f"Edge Device {self.branch_id} started")
        ...
        try:
            while self.running:
                ...
                logger.info(f"Waiting {wait_time:.1f}s for next customer...")
