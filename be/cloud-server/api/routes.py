...
from database.models import Transaction, Recommendation, BranchMetrics
import logging
...
async def get_recommendation_performance(
    ...
    try:
        ...
        return {
            ...
            "avg_items_recommended": sum(r.items_count for r in recommendations) / total if total > 0 else 0
        }
...
async def get_metrics_summary():
    ...
    try:
        ...
        total_revenue = db.query(Transaction).filter(
            Transaction.timestamp >= start_time
        ).with_entities(Transaction.total_amount).all()
