...
class Settings(BaseSettings):
    ...
    POSTGRES_DB: str = Field(default="localhost", env="POSTGRES_HOST")
    ...
    FACE_IMAGE_RETENTION_HOURS: int = Field(
        default=0, env="FACE_IMAGE_RETENTION_HOURS"
    )  # 0 = don't store
    ...
    JWT_SECRET_KEY: str = Field(
        default="change-this-secret-key-in-production", env="JWT_SECRET_KEY"
    )
    ...
    EDGE_HEARTBEAT_INTERVAL_SECONDS: int = Field(
        default=60, env="EDGE_HEARTBEAT_INTERVAL_SECONDS"
    )
    EDGE_OFFLINE_THRESHOLD_MINUTES: int = Field(
        default=5, env="EDGE_OFFLINE_THRESHOLD_MINUTES"
    )
